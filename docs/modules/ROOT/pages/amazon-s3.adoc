...
The easiest way to start working with S3 is to run a local instance using Dev Services. This will automatically start a S3 enabled localstack
instance when running in dev or test mode. Unlike other dev services S3 dev services must be explicitly enabled by setting the `quarkus.s3.devservices.enabled=true`
config property.

When this property is set Quarkus will automatically start and configure a local S3 instance for you, with no other config required. You can optionally configure
the buckets that are created on startup with the `quarkus.s3.devservices.buckets` config property.

Note that data will persist between restarts, so tests must be careful to clean up after themselves.
