...
[source,properties]
----
quarkus.stepfunctions.sync-client.type=apache
----
...
[source,properties]
----
quarkus.stepfunctions.endpoint-override=http://localhost:8083

quarkus.stepfunctions.aws.region=us-east-1
quarkus.stepfunctions.aws.credentials.type=static
quarkus.stepfunctions.aws.credentials.static-provider.access-key-id=test-key
quarkus.stepfunctions.aws.credentials.static-provider.secret-access-key=test-secret
----

- `quarkus.stepfunctions.aws.region` - It's required by the client, but since you're using a local SFN instance you can pick any valid AWS region.
- `quarkus.stepfunctions.aws.credentials.type` - Set `static` credentials provider with any values for `access-key-id` and `secret-access-key`
- `quarkus.stepfunctions.endpoint-override` - Override the SFN client to use a local instance instead of an AWS service

If you want to work with an AWS account, you'd need to set it with:
[source,properties]
----
quarkus.stepfunctions.aws.region=<YOUR_REGION>
quarkus.stepfunctions.aws.credentials.type=default
----

- `quarkus.stepfunctions.aws.region` you should set it to the region where you provisioned the SFN table,
- `quarkus.stepfunctions.aws.credentials.type` - use the `default` credentials provider chain that looks for credentials in this order:
...
[source,properties]
----
quarkus.stepfunctions.async-client.type=aws-crt
----
