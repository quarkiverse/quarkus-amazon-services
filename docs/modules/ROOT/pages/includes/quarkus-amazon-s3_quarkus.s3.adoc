...
a|icon:lock[title=Fixed at build time] [[quarkus-amazon-s3_quarkus-s3-unremovable-s3-presigner-bean]] [.property-path]##link:#quarkus-amazon-s3_quarkus-s3-unremovable-s3-presigner-bean[`quarkus.s3.unremovable-s3-presigner-bean`]##

[.description]
--
When `true`, a `S3Presigner` default bean is produced even if no injection points are discovered. Note: The default value ensures backward compatibility; you can disable it. It will be removed in the next major version.


ifdef::add-copy-button-to-env-var[]
Environment variable: env_var_with_copy_button:+++QUARKUS_S3_UNREMOVABLE_S3_PRESIGNER_BEAN+++[]
endif::add-copy-button-to-env-var[]
ifndef::add-copy-button-to-env-var[]
Environment variable: `+++QUARKUS_S3_UNREMOVABLE_S3_PRESIGNER_BEAN+++`
endif::add-copy-button-to-env-var[]
--
|boolean
|`true`
