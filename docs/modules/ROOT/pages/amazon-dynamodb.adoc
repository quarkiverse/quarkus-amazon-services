...
The Quarkus extension supports the DynamoDb client as well as the https://docs.aws.amazon.com/sdk-for-java/latest/developer-guide/dynamodb-enhanced-client.html[enhanced client].
...
The DynamoDB enhanced client is part of the AWS SDK for Java version 2 and makes the mapping between DynamoDB Tables and Java beans very easy. The Enhanced Client
requires a TableSchema, which can be generated with a set of annotations or declared in code. It is exists as sync and async variant.

To use the enhanced client you additionally need to add `quarkus-amazon-dynamodb-enhanced` to your pom.xml.
...
TIP: `@RegisterForReflection` could also be removed because Quarkus automatically registers Beans annotated with `@DynamoDbBeans`.
...
More information according to the DynamoDb enhanced client:
